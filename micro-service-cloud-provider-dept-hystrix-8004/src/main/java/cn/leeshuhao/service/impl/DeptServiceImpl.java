...
//        int outTime = 6;
    ...
    @HystrixCommand(fallbackMethod = "deptCircuitBreakerFallback", commandProperties = {
            ...
            @HystrixProperty(name = "circuitBreaker.sleepWindowInMilliseconds", value = "10000"), //休眠时间窗口期 休眠时间窗，熔断开启状态持续一段时间后，熔断器会自动进入半熔断状态，这段时间就被称为休眠窗口期。
            //错误百分比阈值。
    ...
    public String deptCircuitBreaker(Integer id) {
        if (id < 0) {
            ...
            throw new RuntimeException("leeshuhao提醒您，id 不能是负数！");
        }
    ...
    public String deptCircuitBreakerFallback(Integer id) {
        return "leeshuhao提醒您，id 不能是负数,请稍后重试!\t id:" + id;
    }

    // 当服务出现故障后，调用该方法给出友好提示
    public String deptTimeoutHandler(Integer id) {
        return  "您的队友提醒您, 系统繁忙请稍后再试！"+"线程池：" + Thread.currentThread().getName() + "  deptInfo_Timeout,id:   " + id;
    }
}
